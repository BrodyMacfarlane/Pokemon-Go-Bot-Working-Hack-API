...
class PGoApi:
    ...
    def __init__(self,CP_CUTOFF=100):
        ...
        self.CP_CUTOFF = CP_CUTOFF # rrelease for cp < that
        self._req_method_list = []
        # check for ids for specf pkm; get json for ids and names and search for names to add
    ...
    def call(self):
        ...
        try:
            ...
            self.log.info('Server is offline')
    ...
    def heartbeat(self):
        ...
        self.get_player()
        self.get_hatched_eggs() # after mock walking/running; not gps teleportation
        self.get_inventory()
    ...
    def walk_to(self,loc): #location in floats of course...
        ...
        for step in steps:
            for i,next_point in enumerate(get_increments(self._posf,step)):
                ...
                # sleep(5)
                # instantaneous sleeping to imitate running within the range from 15-20 km
                sleep(2)
            ...
            if len(pokemons) > 1:
                ...
                for pokemon in pokemons[1:]:
                    if 'cp' in pokemon and pokemon['cp'] < self.CP_CUTOFF:
                        self.log.info("Released: %s", pokemon)
    ...
    def encounter_pokemon(self,pokemon):
        ...
        self.log.info("Encountered: %s", resp)
        if resp['status'] == 1:
            capture_status = -1
